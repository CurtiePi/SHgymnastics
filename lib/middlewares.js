module.exports = {
  ...
  confirmPwdEq: function(req, res, next) {
    console.log('Checking the confirmation password equals the main password');

    let confirmpwd = req.body.confirmpwd;
    req.checkBody('confirmpwd', 'Passwords do not match').equals(req.body.newpwd); 
    var errors = req.validationErrors();
    console.log('Parameters have been checked.');
    if(errors) {
    console.log('Errors have been found. Redirect!');
